...
disp('visualfeedback1'); save temp.mat 
if trainTest == 1;
switch fb
    case 1, %correct
...
disp('visualfeedback2'); save temp.mat         
        Screen('DrawTexture',window,feedback.correct,[],[where.xc-feedback.fb_size(2)/2, ...
                                         ...
                                         where.yc+feedback.fb_size(1)/2]);
        correctkey(window,color,where,stim,feedback,trial,trialWithinBlock)
...
disp('visualfeedback3'); save temp.mat         
...
disp('visualfeedback4'); save temp.mat         
    case 0, %incorrect
disp('visualfeedback4.5'); save temp.mat           
        Screen('DrawTexture',window,feedback.wrong,[],[where.xc-feedback.fb_size(2)/2, ...
                                         where.yc-feedback.fb_size(1)/2, ...
                                         where.xc+feedback.fb_size(2)/2, ...
                                         where.yc+feedback.fb_size(1)/2]);
disp('visualfeedback4.6'); save temp.mat                                       
        correctkey(window,color,where,stim,feedback,trial,trialWithinBlock)
disp('visualfeedback5'); save temp.mat         
        [time.fb_begin time.fb_onset time.fb_end Missed Beampos]=...
            Screen('Flip', window, time.response+time.ifi,0); %clearmode = 0; insert one frame delay                                     
...
disp('visualfeedback9'); save temp.mat         
    case -1, %no feedback
        time.fb_end = GetSecs;
        [time.fb_begin time.fb_onset time.fb_end Missed Beampos]=...
...
disp('visualfeedback10'); save temp.mat 
else
        time.fb_end = GetSecs;
        [time.fb_begin time.fb_onset time.fb_end Missed Beampos]=...
            Screen('Flip', window, time.response+time.ifi,0); 
...
disp('visualfeedback11'); save temp.mat 
%display fixation
string = '+';
